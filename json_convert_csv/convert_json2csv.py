...
for mode in modes_list:
    ...
    if DominantSeasonOnly:
      ...
      seasons = ['monthly'] if mode == 'PDO' else ['DJF', 'MAM', 'JJA', 'SON'] 
            ...
            try:
                ...
                for run in sorted(runs_list, key=lambda s:s.lower()):
                    try:
                        ...
                        if model_run not in model_run_list: model_run_list.append(model_run)
                        if model_run not in d2.keys(): d2[model_run] = {} 
                        if mode_season not in d2[model_run].keys(): d2[model_run][mode_season] = {} 
